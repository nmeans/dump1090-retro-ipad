...
then
	cd /tmp
	if ! wget --timeout=30 -q -O master.zip https://github.com/nmeans/dump1090-retro-ipad/archive/master.zip || ! unzip -q -o master.zip
	then
		echo "------------------"
		echo "Unable to download files, exiting! (Maybe try again?)"
		exit 1
	fi
	cd dump1090-retro-ipad-main
fi
...
cp 88-dump1090-retro-ipad.conf /etc/lighttpd/conf-available
lighty-enable-mod dump1090-retro-ipad >/dev/null
